...
object UserControl {
    ...
    private suspend fun insert(username: String, password: String): User? = dbQuery {
        ...
        resultStatement.resultedValues?.singleOrNull()?.let(::resultToUser)
    }
    ...
    private suspend fun getByUsername(username: String): User? = dbQuery {
        Users.select { Users.username.eq(username) }.singleOrNull()
            ?.let(::resultToUser)
    }

    suspend fun login(username: String, password: String): User?  = dbQuery {
        Users.select { Users.username.eq(username) and Users.password.eq(AppUtils.hashString(password)) }.singleOrNull()
            ?.let(::resultToUser)
    }
